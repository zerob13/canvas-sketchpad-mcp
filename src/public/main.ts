...
class CanvasMCPApp {
  ...
  constructor() {
    ...
    try {
      ...
      this.canvasRenderer = new CanvasRenderer(this.domManager.ctx, this.stateManager);
      this.eventsManager = new EventsManager(this.domManager, this.stateManager);
      ...
      if (this.domManager) {
        ...
        this.domManager.showMessage('Application initialization failed: ' + (error as Error).message, 'error');
      }
    }
  }
  ...
  private startSSEConnection(): void {
    try {
      ...
      this.domManager.setConnectionStatus('error');
      this.domManager.showMessage('Failed to connect to MCP server. Using fallback polling.', 'warning');
    }
  }

  private async sendCommandStatus(commandId: string, status: 'executed' | 'error', error?: string): Promise<void> {
    try {
  ...
  public executeCommand(commandString: string): void {
    ...
    try {
      ...
      this.domManager.showMessage(`Executed ${successCount} commands successfully`, 'success');
  ...
  private handleCanvasClick(event: MouseEvent): void {
    ...
    if (action) {
      ...
      try {
        ...
        this.domManager.showMessage(`Action triggered: ${action.eventName}`, 'info');
      } catch (error) {
...
document.addEventListener('DOMContentLoaded', () => {
  try {
    ...
    window.executeCanvasCommand = (command: string) => app.executeCommand(command);
