...
export class EventsManager {
  ...
  private setupEventListeners(): void {
    // Canvas 点击事件
    const canvasClickHandler = (event: MouseEvent) => this.handleCanvasClick(event);
    ...
    const canvasMouseMoveHandler = (event: MouseEvent) => this.handleCanvasMouseMove(event);
    this.domManager.canvas.addEventListener('mousemove', canvasMouseMoveHandler);
    this.eventListeners.set('canvas-mousemove', canvasMouseMoveHandler);

    // Canvas 鼠标离开事件
    const canvasMouseLeaveHandler = () => this.handleCanvasMouseLeave();
    this.domManager.canvas.addEventListener('mouseleave', canvasMouseLeaveHandler);
    this.eventListeners.set('canvas-mouseleave', canvasMouseLeaveHandler);

    // 控制按钮事件
    const clearCanvasHandler = () => this.handleClearCanvas();
    this.domManager.clearCanvasBtn.addEventListener('click', clearCanvasHandler);
    this.eventListeners.set('clear-canvas', clearCanvasHandler);

    const clearHistoryHandler = () => this.handleClearHistory();
    this.domManager.clearHistoryBtn.addEventListener('click', clearHistoryHandler);
    this.eventListeners.set('clear-history', clearHistoryHandler);

    // 键盘快捷键
    const keyboardHandler = (event: KeyboardEvent) => this.handleKeyboard(event);
    document.addEventListener('keydown', keyboardHandler);
    this.eventListeners.set('keyboard', keyboardHandler);

    // 监听来自父窗口的消息
    const messageHandler = (event: MessageEvent) => this.handleWindowMessage(event);
  ...
  private handleCanvasClick(event: MouseEvent): void {
    ...
    console.log(`🖱️ Canvas clicked at (${coords.x.toFixed(1)}, ${coords.y.toFixed(1)})`);
  ...
  private debugInfo(): void {
    ...
    console.log('Registered Actions:', this.stateManager.registeredActions.length);
  ...
  // 公共API方法
  public addEventListener(eventType: string, handler: (event: CustomEvent) => void): void {
    document.addEventListener(eventType, handler as EventListener);
  }

  public removeEventListener(eventType: string, handler: (event: CustomEvent) => void): void {
    ...
    for (const [eventType, handler] of this.eventListeners) {
      switch (eventType) {
        ...
        case 'canvas-mouseleave':
          this.domManager.canvas.removeEventListener(eventType.replace('canvas-', ''), handler);
          break;
        case 'clear-canvas':
