...
export class DOMManager {
  ...
  constructor() {
    ...
    this.commandHistory = document.getElementById('command-history') as HTMLDivElement;
    this.clearHistoryBtn = document.getElementById('clear-history-btn') as HTMLButtonElement;
    this.clearCanvasBtn = document.getElementById('clear-canvas-btn') as HTMLButtonElement;
    this.connectionStatus = document.getElementById('connection-status') as HTMLSpanElement;

    // Get status display elements
    this.parseStatus = document.getElementById('parse-status') as HTMLSpanElement;
    this.commandCount = document.getElementById('command-count') as HTMLSpanElement;
    this.actionCount = document.getElementById('action-count') as HTMLSpanElement;
    this.lastExecution = document.getElementById('last-execution') as HTMLSpanElement;
    this.messageArea = document.getElementById('message-area') as HTMLDivElement;
    this.messageContent = document.getElementById('message-content') as HTMLDivElement;
  ...
  showMessage(message: string, type: 'success' | 'error' | 'info' = 'info'): void {
    ...
    switch (type) {
      case 'success':
        this.messageContent.classList.add('bg-green-50', 'text-green-800', 'border', 'border-green-200');
        break;
      case 'error':
        this.messageContent.classList.add('bg-red-50', 'text-red-800', 'border', 'border-red-200');
        break;
      case 'info':
      default:
        this.messageContent.classList.add('bg-blue-50', 'text-blue-800', 'border', 'border-blue-200');
        break;
    }
  ...
  addCommandToHistory(command: string, status: 'success' | 'error' | 'executing' = 'success'): void {
    ...
    switch (status) {
      case 'success':
        historyItem.classList.add('border-green-400', 'bg-green-50', 'text-green-800');
        break;
      case 'error':
        historyItem.classList.add('border-red-400', 'bg-red-50', 'text-red-800');
        break;
      case 'executing':
        historyItem.classList.add('border-blue-400', 'bg-blue-50', 'text-blue-800');
        break;
    }
    ...
    if (placeholder && placeholder.textContent?.includes('Waiting for MCP commands')) {
